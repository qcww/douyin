...
var scrapUser = '';
var scripHost = 'http://dyapi.bjbctx.com/';
...
function toFansHome(uid){
    while(!className('android.widget.Button').desc('搜索').exists()){
        back();
        sleep(2000);
    }
    
    className('android.widget.Button').desc('搜索').click();
    sleep(2000);
    var searchOb = className('android.widget.EditText').findOne(3000).bounds();
    click(searchOb.centerX(),searchOb.centerY());
    sleep(1500);
    setText(uid);
    sleep(1500);
    click(width*0.9,searchOb.centerY());
    sleep(2000);
    if(className("android.widget.TextView").textStartsWith("抖音号:").exists()){
        click('抖音号:')
    }
}
...
function say(said){
    ...
    // 粘贴板没有内容，重试三次，不行跳过
    var tt = 3;
    while(!className("android.widget.TextView").textStartsWith("打开看看").findOne(15000)){
        setClip('');
        tt--;
    ...
    cons.log('开始打招呼操作');
    className('android.widget.ImageView').desc('更多').click()
  
    sleep(2000);
    className('android.widget.ImageView').desc('私信').click()
    sleep(2000)
