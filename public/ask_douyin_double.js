...
// 抖音软件坐标
var appSign = new Array({x:0.15,y:0.9},{x:0.85,y:0.9});
...
function startApp(){
    home();
    sleep(500);
    click(width*appSign[selectApp].x,height*appSign[selectApp].y);
    return true;
}
...
while(true){
    ...
    if(firstAsk) doRest(false);

    list = getScrapList(getFansUrl)
    checkNews = true;
    // 一直休息到最近的下个时间节点开始打招呼
    if(isWait && !firstAsk && selectApp == 0){
